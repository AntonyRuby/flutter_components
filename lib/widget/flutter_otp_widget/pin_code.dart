...
import 'package:flutter_components/widget/flutter_otp_widget/haptic_dialog.dart';
import 'package:flutter_components/widget/flutter_otp_widget/pin_code_cursor_painter.dart';
import 'package:flutter_components/widget/flutter_otp_widget/pin_code_haptic.dart';
...
class FlutterPinCodeTextField extends StatefulWidget {
  ...
  final FlutterPinTheme pinTheme;
  ...
  /// The vertical padding from the [FlutterPinCodeTextField] to the error text
  /// Default is 16.
  ...
  FlutterPinCodeTextField({
    ...
    this.dialogConfig,
    this.pinTheme = const FlutterPinTheme(),
  ...
  @override
  _FlutterPinCodeTextFieldState createState() =>
      _FlutterPinCodeTextFieldState();
}

class _FlutterPinCodeTextFieldState extends State<FlutterPinCodeTextField>
  ...
  FlutterPinTheme get pinTheme => widget.pinTheme;
  ...
  void _checkForInvalidValues() {
    ...
    assert(pinTheme.fieldHeight > 0);
    assert(pinTheme.fieldWidth > 0);
    assert(pinTheme.borderWidth >= 0);
  ...
  void debounceBlink() {
    ...
    if (widget.blinkWhenObscuring &&
      ...
      if (blinkDebounce?.isActive ?? false) {
        blinkDebounce!.cancel();
      }
      blinkDebounce = Timer(widget.blinkDuration, () {
        setState(() {
          _hasBlinked = true;
        });
      });
  ...
  Color _getColorFromIndex(int index) {
    ...
    if (((_selectedIndex == index) ||
      ...
      Color relevantActiveColor = pinTheme.activeColor!;
      if (isInErrorMode) relevantActiveColor = pinTheme.errorBorderColor!;
      return relevantActiveColor;
    }

    Color relevantInActiveColor = pinTheme.inactiveColor!;
    if (isInErrorMode) relevantInActiveColor = pinTheme.errorBorderColor!;
...
// selects the right fill color for the field
  Color _getFillColorFromIndex(int index) {
    ...
    if (((_selectedIndex == index) ||
      ...
      return pinTheme.activeFillColor!;
    }
